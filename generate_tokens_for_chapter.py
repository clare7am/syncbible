...
def build_tokens_for_chapter(book_id: int, chapter: int):
    ...
    # ✅ 从 books 表查缩写
    cursor.execute("SELECT abbr FROM books WHERE id = ?", (book_id,))
    row = cursor.fetchone()
    if not row:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("用法：python build_tokens.py <book_id> <chapter>")
        print("示例：python build_tokens.py 1 6")
        sys.exit(1)

    book_id = int(sys.argv[1])
    chapter = int(sys.argv[2])

    build_tokens_for_chapter(book_id, chapter)s