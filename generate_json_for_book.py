...
def get_book_chapter_map(conn):
    """
    从数据库查询每卷书有多少章
    返回：{book_id: max_chapter, ...}
    """
    cur = conn.cursor()
    rows = cur.execute("""
        SELECT book_id, MAX(chapter) AS max_chapter
        FROM verses
        GROUP BY book_id
        ORDER BY book_id
    """).fetchall()

    return {row["book_id"]: row["max_chapter"] for row in rows}


def generate_chapter_json(conn, book_id, chapter):
...
if __name__ == "__main__":
    ...
    # 查出每卷书有多少章
    book_chapter_map = get_book_chapter_map(conn)

    # 外层：遍历 73 卷书
    for book_id, max_chapter in book_chapter_map.items():
        # 内层：遍历每一章
        for chapter in range(1, max_chapter + 1):
            generate_chapter_json(conn, book_id, chapter)
